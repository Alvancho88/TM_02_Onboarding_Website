...
import { Wind, Search, TrendingUp } from "lucide-react"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-sky-900">
                  Malaysia Air Quality Monitor
                </h1>
                <p className="text-sky-600 text-sm md:text-base">
                  Real-time Air Pollution Index (API) for Your Area
                </p>
              </div>
            </div>
            <Link 
              href="/trends"
              className="flex items-center gap-2 px-4 py-2 bg-sky-50 hover:bg-sky-100 rounded-lg text-sky-700 font-medium transition-colors"
            >
              <TrendingUp className="h-5 w-5" />
              <span className="hidden md:inline">View Trends</span>
            </Link>
          </div>
        </div>
      </header>
        ...
        <Card className="bg-white shadow-lg border-sky-100 overflow-hidden">
          <CardContent className="p-0">
            ...
            <div className="h-[500px] md:h-[600px]">
              <MalaysiaMap
                markers={Object.entries(mockApiData).map(([name, data]) => ({
                  name,
                  api: data.api,
                  lat: data.lat,
                  lng: data.lng,
                ...
                onMarkerClick={(marker) => {
                  ...
                  const stateData = malaysiaStates.find(s => s.areas.includes(marker.name))
                  if (stateData) {
                    router.push(`/location?state=${encodeURIComponent(stateData.name)}&area=${encodeURIComponent(marker.name)}`)
                  }
                }}
              />
            </div>
